...
void LobotSerialServoControl::lobotSerialServoOffsetWrite(uint8_t id, int8_t offset) {
  uint8_t buf[7];  // Adjusted to the correct size

  buf[0] = LOBOT_SERVO_FRAME_HEADER;
  buf[1] = LOBOT_SERVO_FRAME_HEADER;
  buf[2] = id;
  buf[3] = 4;  // Length of the command
  buf[4] = LOBOT_SERVO_ANGLE_OFFSET_ADJUST;
  buf[5] = (uint8_t)offset;  // Convert signed to unsigned
  buf[6] = LobotCheckSum(buf);  // Calculate checksum

  if (isAutoEnableRT == false)
    TxEnable();

  SerialX->write(buf, 7);  // Send the buffer
}
