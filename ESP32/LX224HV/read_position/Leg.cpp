...
double Leg::range_limits[6] = { -30, 30, -90, 90, -0.1, 105 };
...
int Leg::leg_to_position(double x, double y, double z) {
  if (verbose) {
...
double* Leg::leg_get_cur_local_pos() {
  if (verbose) {
    Serial.printf("-------------------- Leg %i servos positions --------------------\n", leg_get_id());
