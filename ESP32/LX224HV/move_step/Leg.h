...
private:
  ...
  int kneeId;
  int ankleId;
  double servo_angles[3]; // servo angle of hip, knee, and ankle
  double servo_units[3]; // servo unit of hip, knee, and ankle
  double leg_positions[3]; // leg tip position x, y, z
...
public:
  ...
  bool leg_to_position(double targetX, double targetY, double targetZ);
  bool leg_step_to_position(double targetX, double targetY, double targetZ);
  ...
  double* leg_get_cur_local_units();
  void leg_check_angle_error();
  
protected:
  bool leg_check_range_limits(double* input_angles);
  void leg_update_units();
  void leg_update_angles();
  void leg_update_positions();
  void leg_update_all();
